...
CHANNEL *channel_register(char *name, channel_callback callback, void *private) {
	CHANNEL *ch;
	int id;
...
void channel_unregister(CHANNEL *ch) {
	...
	DSFYDEBUG("channel %d: unregistering, %d headers, %u bytes header, %u bytes payload\n",
		ch->channel_id, ch->header_id, ch->total_header_len, ch->total_data_len);
...
int channel_process(unsigned char *buf, unsigned short len, int error) {
	...
	if(ch == NULL) {
		DSFYDEBUG("channel_process(): Failed to find channel for id=%u (error flag: %d), lost %u bytes data\n",
	...
	DSFYDEBUG("channel %d: Entering callback (state: %s) for channel '%s', %d bytes data\n",
		ch->channel_id, ch->state == CHANNEL_DATA? "data":
		ch->state == CHANNEL_ERROR? "error": "end", ch->name, len);
