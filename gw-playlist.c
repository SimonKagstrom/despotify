...
static int gw_getplaylist_result_callback(CHANNEL *, unsigned char *, unsigned short);
...
int gw_getplaylist(SPOTIFYSESSION *s, char *playlist_hex_id) {
	...
	return cmd_getplaylist(s->session, id, -1, gw_getplaylist_result_callback, (void *)s);
}


static int gw_getplaylist_result_callback(CHANNEL *ch, unsigned char *buf, unsigned short len) {
