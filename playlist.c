...
static struct track *playlist_track_by_id(struct playlist *, unsigned char *);
static void playlist_xml_handle_startelement(void *, const XML_Char *, const XML_Char **);
...
static void tracks_meta_xml_handle_startelement(void *, const XML_Char *, const XML_Char **);
...
struct track *playlist_track_add(struct playlist *p, unsigned char *id) {
	...
	if((t = p->tracks) == NULL) {
		p->tracks = t = (struct track *)calloc(1, sizeof(struct track));
	}
	else {
		while(t->next)
			t = t->next;
...
static struct track *playlist_track_by_id(struct playlist *p, unsigned char *id) {
	struct track *t;
...
int playlist_create_from_xml(char *xmldata, struct playlist *target) {
	...
	XML_SetElementHandler(ctx.p, playlist_xml_handle_startelement, playlist_xml_handle_endelement);
...
int playlist_track_update_from_gzxml(struct playlist *target, void *data, int len) {
	int ret;
	struct parsingctx ctx;
	...
	XML_SetElementHandler(ctx.gzxml->p, tracks_meta_xml_handle_startelement, tracks_meta_xml_handle_endelement);
	XML_SetCharacterDataHandler(ctx.gzxml->p, tracks_meta_xml_handle_text);
...
static void playlist_xml_handle_endelement(void *private, const XML_Char *name) {
	struct parsingctx *ctx = (struct parsingctx *)private;

	/* Mark loading of playlist complete */
	if(!ctx->list_playlists
		&& xml_has_parent_path(ctx->taglist, "//next-change/change/ops")
		&& !strcmp(name, "create")) {
...
static void playlist_xml_handle_text(void *private, const XML_Char *s, int len) {
	struct parsingctx *ctx = (struct parsingctx *)private;
	...
	if(!ctx->list_playlists
		&& (ts = xml_has_parent_path(ctx->taglist, "//next-change/change")) != NULL
		&& !strcmp(ts->name, "user")) {

		playlist_set_author(ctx->pl, (char *)buf);
	}
	else if(!ctx->list_playlists
		&& (ts = xml_has_parent_path(ctx->taglist, "//next-change/change/ops")) != NULL
		&& !strcmp(ts->name, "name")) {

		playlist_set_name(ctx->pl, (char *)buf);
	}
	else if((ts = xml_has_parent_path(ctx->taglist, "//next-change/change/ops/add")) != NULL
		&& !strcmp(ts->name, "items")) {

		ptr = buf;
		while(len > 0) {
			while(len && (*ptr == ' ' || *ptr == '\r' || *ptr == '\n'))
				ptr++, len--;

			ptr_next = ptr;
			while(len && ((*ptr_next >= '0' && *ptr_next <= '9') || (*ptr_next >= 'a' && *ptr_next <= 'f')))
				ptr_next++, len--;
...
static void tracks_meta_xml_handle_startelement(void *private, const XML_Char *name, const XML_Char **attr) {
	struct parsingctx *ctx = (struct parsingctx *)private;
	int i;

	xml_push_tag(&ctx->taglist, name);

	if(xml_has_parent_path(ctx->taglist, "//tracks/track/files") && !strcmp(name, "file")) {
		for(i = 0; attr[i]; i += 2) {
			if(strcmp(attr[i], "id"))
				continue;

			if(ctx->track)
				hex_ascii_to_bytes((char *)attr[i + 1], ctx->track->file_id, 20);
			break;
		}
	}
}
...
static void tracks_meta_xml_handle_endelement(void *private, const XML_Char *name) {
	...
	if(xml_has_parent_path(ctx->taglist, "//tracks")) {
		...
		if(ctx->track) {
			...
			if(!memcmp(ctx->track->file_id, invalid_file_id, 20))
				playlist_track_del(ctx->pl, ctx->track->track_id);
		}
...
static void tracks_meta_xml_handle_text(void *private, const XML_Char *s, int len) {
	struct parsingctx *ctx = (struct parsingctx *)private;
	struct tagstack *ts = ctx->taglist;
	int i;
	char *buf;
