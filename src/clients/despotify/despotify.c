...
static int despotify_no_support_for_introduction_accounts (EVENT *,
							   enum ev_flags);
...
static int despotify_login (EVENT * ev, enum ev_flags ev_kind)
{
	...
	case 3:
		...
		key_init (s);
		puzzle_solve (s);
		auth_generate_auth_hmac (s, s->auth_hmac,
					 sizeof (s->auth_hmac));
	...
	case 5:
		...
		ev = event_register_action (NULL, 0, despotify_packet_io,
					    (void *) s);
		event_register_fd (ev, s->ap_sock);
		...
		if (!is_initialized++) {
			...
			*container = s;
			...
			event_msg_post (MSG_CLASS_GUI, MSG_GUI_SESSIONPTR,
					container);

			/* Register handler for downloading playlists */
			ev = event_register_action (NULL, 0,
						    gui_playlists_download,
						    NULL);
			event_msg_subscription_class_set (ev,
							  MSG_CLASS_APP |
							  MSG_CLASS_GUI);
...
static int despotify_packet_io (EVENT * ev, enum ev_flags ev_kind)
{
	...
	if (ev_kind == EV_MSG) {
		if (ev->msg->class == MSG_CLASS_APP) {
			...
			case MSG_APP_NET_ERROR:
				DSFYDEBUG
					("despotify_packet_io(): MSG_APP_NET_ERROR, disconnecting..\n");
				/* Disconnect */
				session_disconnect (s);
				event_msg_post (MSG_CLASS_APP,
						MSG_APP_DISCONNECTED, NULL);
	...
	if (err) {
		...
		ev = event_register_action (NULL, 0, despotify_login,
					    ev->private);
		event_msg_subscription_class_set (ev, MSG_CLASS_APP);
	}
...
static int despotify_no_support_for_introduction_accounts (EVENT * ev,
							   enum ev_flags
							   ev_kind)
{
