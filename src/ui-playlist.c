...
static int gui_playlist_channel_callback(CHANNEL *, unsigned char *, unsigned short);
...
void gui_playlist_display(WINDOW *w, struct playlist *p) {
	...
	for(i = 1, t = p->tracks; i < (y - 2 - 2) && t; i++, t = t->next) {
		...
		wattroff(w, A_BOLD);
		mvwprintw(w, 2 + i, 7, "%.24s - %.22s [%.24s]", t->title, t->artist, t->album);
	}
...
int gui_playlists_download(EVENT *e, enum ev_flags ev_kind) {
	...
	case 1:
		...
		if((err = cmd_getplaylist(session, (unsigned char *)PLAYLIST_LIST_PLAYLISTS,
						-1, gui_playlist_channel_callback,
						(void *)r)) == 0) {
			e->state++;
	...
	case 2:
		...
		for(p = *rootptr; p; p = p->next)
			if((p->flags & (PLAYLIST_LOADED|PLAYLIST_ERROR)) == 0)
				break;
		...
		buffer_append_raw(r->response, "<?xml version=\"1.0\" encoding=\"utf-8\" ?>\n<playlist>\n", 51);
		if((err = cmd_getplaylist(session, r->playlist->playlist_id,
						-1, gui_playlist_channel_callback,
						(void *)r)) == 0) {
	...
	case 3:
		...
		rootptr = playlist_root();
		for(p = *rootptr; p; p = p->next) {
			if((p->flags & (PLAYLIST_TRACKS_LOADED|PLAYLIST_ERROR)) == 0) {
				if(p->num_tracks != 0)
					break;
		...
		for(p->num_tracks = 0, t = p->tracks; t; p->num_tracks++, t = t->next)
			if(p->num_tracks < 128)
				memcpy(r->track_id_list + 16*p->num_tracks, t->track_id, 16);

		if((err = cmd_browse(session, 3, r->track_id_list, p->num_tracks,
					gui_playlist_channel_callback, (void *)r)) == 0) {
			/* Make sure we're not called until the message processing code marks us busy again */
			event_mark_idle(e);
		}
		else {
...
static int gui_playlist_channel_callback(CHANNEL *ch, unsigned char *buf, unsigned short len) {
	...
	case CHANNEL_ERROR:
		...
		*container = r;
		...
		if(r->track_id_list)
			event_msg_post(MSG_CLASS_GUI, MSG_GUI_PLAYLIST_BROWSE_ERROR, container);
		else if(r->playlist == NULL)
			event_msg_post(MSG_CLASS_GUI, MSG_GUI_PLAYLIST_LIST_ERROR, container);
		else
			event_msg_post(MSG_CLASS_GUI, MSG_GUI_PLAYLIST_TRACKS_ERROR, container);
	...
	case CHANNEL_END:
		...
		*container = r;
		if(r->track_id_list)
			event_msg_post(MSG_CLASS_GUI, MSG_GUI_PLAYLIST_BROWSE_OK, container);
		else if(r->playlist == NULL)
			event_msg_post(MSG_CLASS_GUI, MSG_GUI_PLAYLIST_LIST_OK, container);
		else
			event_msg_post(MSG_CLASS_GUI, MSG_GUI_PLAYLIST_TRACKS_OK, container);
