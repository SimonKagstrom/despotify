...
int main(int argc, char **argv) {
	...
	do {
		...
		for(i = 0; i < num_clients; i++) {
			...
			if(client[i]->last_activity + SPOTIFYSESSION_TIMEOUT > t) {
				if(spotify_fsm(client[i]) == 0) {
					if(client[i]->state != CLIENT_STATE_IDLE_CONNECTED
						&& client[i]->state != CLIENT_STATE_COMMAND_COMPLETE)
						ok_to_idle = 0;
					continue;
				}
			}
		...
		if(FD_ISSET(listen_fd, &rfds)) {
			...
			if((rest = realloc(rest, (num_rest+1) * sizeof(RESTSESSION *))) == NULL)
				exit(1);
		...
		for(i = 0; i < num_clients; i++)
			if(client[i]->session->ap_sock != -1
				&& FD_ISSET(client[i]->session->ap_sock, &rfds))
...
int rest_read_input(RESTSESSION *r) {
	...
	do {
		...
		ret = read(r->socket, r->input + r->input_len, nbytes_to_read);
		if(ret <= 0)
			return -1;
...
int rest_fsm(RESTSESSION *r) {
	...
	case REST_STATE_PROCESS_INIT_COMMAND:
		if(!strcmp(r->command, "quit")) {
			...
			ret = -1;
			if((r->httpreq = malloc(sizeof(HTTPREQUEST))) != NULL) {
				temp[255] = 0;
		...
		else if((ret = sscanf(r->command, "login %255s %255s", r->username, r->password)) == 2) {
			...
			r->state = REST_STATE_LOAD_COMMAND;
			if(r->client)
				sprintf(msg, "200 0 OK Client with id '%.100s' found\n", temp);
			else
	...
	case REST_STATE_PROCESS_HTTP_REQUEST:
		...
		if(strlen(r->command) == 0) {
			...
			if(!strncasecmp(r->command, "Authorization: Basic ", 21)) {
				if(r->httpreq->authheader)
					free(r->httpreq->authheader);

				r->httpreq->authheader = strdup(r->command + 21);
			}
	...
	case REST_STATE_PROCESS_CLIENT_COMMAND:
		if(!strcmp(r->command, "quit")) {
			...
			r->state = REST_STATE_WAITING;
			if((ret = gw_search(r->client, r->command + 7)) != 0)
				r->state = REST_STATE_FREE_CLIENT;
		}
		else if(sscanf(r->command, "key %40s %32s", temp, temp2) == 2) {
			temp[40] = temp2[32] = 0;
			...
			if(strlen(temp) != 40 || strlen(temp2) != 32) {
				...
				r->state = REST_STATE_WAITING;
				if((ret = gw_file_key(r->client, file_id, track_id)) != 0)
					r->state = REST_STATE_FREE_CLIENT;
			}
		}
		else if(sscanf(r->command, "substream %40s %u %u %32s", temp, &offset, &length, temp2) == 4) {
			temp[40] = temp2[32] = 0;
			...
			if(strlen(temp) != 40 || (offset % 1024) != 0 || len == 0 || strlen(temp2) != 32) {
				...
				r->state = REST_STATE_WAITING;
				if((ret = gw_file_stream(r->client, file_id, offset, length, key)) != 0)
					r->state = REST_STATE_FREE_CLIENT;
			}
		}
			...
			if(strlen(r->command) != 9+2*17) {
				...
				r->state = REST_STATE_WAITING;
				if((ret = gw_getplaylist(r->client, r->command + 9)) != 0)
					r->state = REST_STATE_FREE_CLIENT;
			}
		}
		else {
...
int spotify_fsm(SPOTIFYSESSION *s) {
	...
	case CLIENT_STATE_IDLE_CONNECTED:
		if(s->client_has_data) {
			s->client_has_data = 0;
			if((ret = packet_read(s->session, &hdr, &payload)) != 0) {
...
int spotify_client_allocate(RESTSESSION *r) {
	...
	if(i == num_clients) {
		client = realloc(client, (num_clients+1) * sizeof(SPOTIFYSESSION *));
		client[num_clients] = (SPOTIFYSESSION *)malloc(sizeof(SPOTIFYSESSION));
		client[num_clients]->session = NULL;
		sprintf(client[num_clients]->client_id, "%08lx%x",
			(unsigned int)time(NULL) ^ getpid() ^ (((unsigned long)client) << 7 & 0xffffffff),
			num_clients + 1);
